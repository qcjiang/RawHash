...
/*
* @brief 		Compute the coordinates of a mapping region
*
* @param r 		Mapping region
* @param qlen 	length of the query sequence
* @param a 		Anchors
* 					a[].x: rev<<63 	 | tid<<32 	  | t_pos
* 					a[].y: flags<<40 | q_span<<32 | q_pos
*/
static inline void mm_reg_set_coor(mm_reg1_t *r,
								   int32_t qlen,
								   const mm128_t *a)
								//    int is_qstrand)
{ // NB: r->as and r->cnt MUST BE set correctly for this function to work
	...
	int32_t k = r->as;
	// int32_t q_span = (int32_t)((a[k].y>>RI_ID_SHIFT)&span_mask);
	r->rev = a[k].x>>63; //strand
	r->rid = a[k].x<<1>>33; //remove strand (<<1) then shift 32+1 right (just to get the id)
	// r->rs = (int32_t)a[k].x + 1 > q_span? (int32_t)a[k].x + 1 - q_span : 0; // NB: target span may be shorter, so this test is necessary
	r->rs = (int32_t)a[k].x; // NB: target span may be shorter, so this test is necessary
...
/*
* @brief 			Compute the hash value of a 64-bit integer
*
* @param key 		64-bit integer
*
...
*/
mm_reg1_t *mm_gen_regs(void *km,
					   uint32_t hash,
					   int qlen,
					   int n_u,
					   uint64_t *u,
					   mm128_t *a)
					//    int is_qstrand) // convert chains to hits
{
	//z[i].x: u[i] ^ hash
	//z[i].y: start anchor (as) index of chain_i <<32 | number of anchors in chain_i
	mm128_t *z, tmp;
	mm_reg1_t *r;
	int i, k;
	...
	// sort by score
	z = (mm128_t*)ri_kmalloc(km, n_u * 16);
	for (i = k = 0; i < n_u; ++i) {
		uint32_t h;
		h = (uint32_t)hash64((hash64(a[k].x) + hash64(a[k].y)) ^ hash);
		// u[i] -- higher 32 bits: chain score; lower 32 bits: number of seeds in the chain
		// The ordering is still protected, i.e., if u[i] > u[j], then h(u[i]) > h(u[j]) and so does z[i].x > z[j].x (i.e., the score)
		z[i].x = u[i] ^ h;
	...
	for (i = 0; i < n_u; ++i) {
		mm_reg1_t *ri = &r[i];
		ri->id = i;
		ri->parent = RH_PARENT_UNSET;
		ri->score = ri->score0 = z[i].x >> 32;
		ri->hash = (uint32_t)z[i].x; //lower 32 bits hash? 
...
// void mm_mark_alt(const ri_idx_t *mi, int n, mm_reg1_t *r)
// {
// 	int i;
...
static inline int mm_alt_score(int score,
							   float alt_diff_frac)
{
				...
				//   int is_qstrand)
{
	...
	*r2 = *r;
	...
	r2->as = r->as + n;
	if (r->parent == r->id) r2->parent = RH_PARENT_TMP_PRI;
				   ...
				   float alt_diff_frac) // and compute mm_reg1_t::subsc
{
	...
	cov = (uint64_t*)ri_kmalloc(km, n * sizeof(uint64_t));
	w = (int*)ri_kmalloc(km, n * sizeof(int));
	w[0] = 0, r[0].parent = 0;
	for (i = 1, k = 1; i < n; ++i) {
		mm_reg1_t *ri = &r[i];
				 ...
				 float alt_diff_frac)
{
	...
	if (n <= 1) return;
	aux = (mm128_t*)ri_kmalloc(km, n * 16);
	t = (mm_reg1_t*)ri_kmalloc(km, n * sizeof(mm_reg1_t));
	for (i = n_aux = 0; i < n; ++i) {
...
void mm_sync_regs(void *km,
				  int n_regs,
				  mm_reg1_t *regs) // keep mm_reg1_t::{id,parent} in sync; also reset id
{
	...
	n_tmp = max_id + 1;
	tmp = (int*)ri_kmalloc(km, n_tmp * sizeof(int));
	...
	for (i = 0; i < n_regs; ++i) {
		mm_reg1_t *r = &regs[i];
		r->id = i;
		if (r->parent == RH_PARENT_TMP_PRI)
			r->parent = i;
		else if (r->parent >= 0 && tmp[r->parent] >= 0)
			r->parent = tmp[r->parent];
		else r->parent = RH_PARENT_UNSET;
	}
	ri_kfree(km, tmp);
...
int mm_filter_strand_retained(int n_regs,
							  mm_reg1_t *r)
{
	int i, k;
...
{ // squeeze out regions in a[] that are not referenced by regs[]
	int i, as = 0;
	uint64_t *aux;
	aux = (uint64_t*)ri_kmalloc(km, n_regs * 8);
	for (i = 0; i < n_regs; ++i)
...
ri_seg_t *mm_seg_gen(void *km,
					 uint32_t hash,
					 int n_segs,
					 const int *qlens,
					 int n_regs0,
					 const mm_reg1_t *regs0,
					 int *n_regs,
					 mm_reg1_t **regs,
					 const mm128_t *a)
{
	...
	seg = (ri_seg_t*)ri_kcalloc(km, n_segs, sizeof(ri_seg_t));
	for (s = 0; s < n_segs; ++s) {
		seg[s].u = (uint64_t*)ri_kmalloc(km, n_regs0 * 8);
		for (i = 0; i < n_regs0; ++i)
	...
	for (i = 0; i < n_regs0; ++i) {
		...
		for (j = 0; j < r->cnt; ++j) {
			int sid = (a[r->as + j].y&RI_SEED_SEG_MASK)>>RI_SEED_SEG_SHIFT;
			++seg[sid].u[i];
			++seg[sid].n_a;
		}
	}
	for (s = 0; s < n_segs; ++s) {
		...
		for (i = 0, sr->n_u = 0; i < n_regs0; ++i) // squeeze out zero-length per-segment chains
			if ((int32_t)sr->u[i] != 0)
				sr->u[sr->n_u++] = sr->u[i];
		sr->a = (mm128_t*)ri_kmalloc(km, sr->n_a * sizeof(mm128_t));
		sr->n_a = 0;
	}
...
void mm_seg_free(void *km,
				 int n_segs,
				 ri_seg_t *segs)
{
	int i;
	for (i = 0; i < n_segs; ++i) ri_kfree(km, segs[i].u);
	for (i = 0; i < n_segs; ++i) ri_kfree(km, segs[i].a);
	ri_kfree(km, segs);
}
